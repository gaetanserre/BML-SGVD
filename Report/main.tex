...
\mainmatter
\titlerunning{Non-asymptotic Stein Variational Gradient Descent}
...
\toctitle{Non-asymptotic Stein Variational Gradient Descent}
...
should solve the issue, but it would require too much particles to be
...
Finally, even though the experiences are given in annex, they link their experiences with a GitHub repository which is not easy to understand at all, and therefore is not reproducible. Also, it is based on the code from \cite{Original-SVGD}, which does not describe properly the steps in the code. \newline
