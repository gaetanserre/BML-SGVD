...
Finally, even though the experiments are given in annex, they link their it with a GitHub repository, based on the code from \cite{Original-SVGD}, which is not easy to understand, and therefore not reproducible. Indeed, it implements a lot of steps that are not clearly described in any of the three main papers (see \cite{Original-SVGD}, \cite{main-paper} and \cite{SVGD-flow}). \newline
