...
We fix here $\pi$ an objective probability distribution, and an initial ditribution $\mu_0$
over a set $\X$.\\
In all what follows, $\X = \R^d$, and both $\mu$ and $\pi$ admit a
density that will also be denoted by $\mu$ and $\pi$ respectively.

\subsection{Notations}
We will denote by $\P_2(\X)$ the set of probability measure on $\X$ with finite second order moment i.e. the set of distributions such that $\int ||x||^2 d\mu(x) < \infty$. We assume the objective distribution $\pi$ lives in $\P_2(\X)$, and define the Kullback-Liebler divergence between $\pi_1$ and $\pi_2$ by
